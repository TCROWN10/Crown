...
export function EventCard({ event }: EventCardProps) {
  ...
  const handlePurchaseTicket = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (userBalance < requiredAmount) {
        toast.error(`Insufficient balance. You need ${event.price} HBAR to purchase this ticket.`)
        setPurchasing(false)
        return
      }
      ...
      try {
        ...
        // Show success message even if WebSocket fails
        toast.success(`🎫 Ticket purchase initiated! Check your wallet for confirmation.`)
        
        // Reset purchasing state
        setPurchasing(false)
        
        // Optional: Refresh the page after a delay to update UI
        setTimeout(() => {
  ...
  const isCorrectNetwork = !isConnected || chainId // Hedera testnet - allow if not connected
  ...
  const handleNetworkSwitch = (e: React.MouseEvent) => {
    e.stopPropagation()
    toast.error(`⚠️ Please switch to Hedera testnet (Chain ID: ${chainId}) to purchase tickets`)
  }
